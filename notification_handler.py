...
advisor_table = advisor_invite_base.get_table()
...
class Invite:
    ...
    def get_invite_member(self, self_id:str):
        invites = member_table.filter(lambda x: x["receiver_id"] == self_id).select(['project_id', "respond", "invite_id"])
        if member_table.filter(lambda x: x["receiver_id"] == self_id).table == []:
            return []
        else:
            return invites # list(map(lambda x: x["project_id"], invites))
    ...
    def accept_invite_member(self, invite_id:str):
        ...
        _data_rest = _data = member_table.filter(lambda x: x['receiver_id'] != invite_data[0]["receiver_id"]).select(["invite_id", "project_id", "receiver_id", "respond", "res_date"])
        _data_new = []
        for i in _data:
            i["respond"] = "rejected"
            _data_new.append(i)
        invite_data += _data_new
        invite_data += _data_rest
        member_table.table = invite_data
        member_invite_base.overide_new_table(member_table)
        project_handler.Project_handler(invite_data[0]["project_id"]).add_member(person_handler.Person(invite_data[0]['receiver_id']))
        return None
    ...
    def get_invite_advisor(self, self_id:str):
        invites = advisor_table.filter(lambda x: x["receiver_id"] == self_id).select(['project_id', "respond", "invite_id"])
    ...
    def accept_invite_advisor(self, invite_id:str):
        ...
        advisor_invite_base.overide_new_table(advisor_table)
        project_handler.Project_handler(invite_data[0]["project_id"]).add_advisor(invite_data[0]['receiver_id'])
